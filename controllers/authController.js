...
const jwtSecret = process.env.JWT_SECRET_KEY || "fallback_secret_key_for_development";
...
export const userRegister = async (req, res) => {
  try {
    ...
    // Handle case where email might be undefined or not a string
...
export const artistRegister = async (req, res) => {
  try {
    ...
    const existingArtist = await Artist.findOne({ where: { email: normalizedEmail } });
...
export const artistLogin = async (req, res) => {
  try {
    ...
    const token = jwt.sign({ artistId: artist.id, email }, JWT_SECRET, {
      expiresIn: "2h",
    });
    artist.token = token;
    artist.password = undefined;
